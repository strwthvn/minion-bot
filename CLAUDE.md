...
- **Reaction-role cleanup**: a record whose message is gone is dropped two ways — `reactionRoleCleanupHandler` listens to `messageDelete`/`messageDeleteBulk`/`channelDelete` (all three are needed: a bulk purge emits no per-message event, and deleting a channel emits neither), and `resolveRecordMessage()` in `reactionrole.js` forgets the record lazily on first use, covering deletions that happened while the bot was offline. **Only ever drop a record on API code 10003/10008** — treating any other fetch failure as "gone" would delete live dispensers whenever the bot briefly loses channel access.
